...
//   // ✅ Insert user with friend data structure
...
//     activeTab: 'home',
//     showProfilePrompt: req.session.showProfilePrompt,
//     requestCount
//   });
// });
...
//     username: { $in: currentUser.friendRequests || [] }
//   }).toArray();
...
//     const isPending = (currentUser.friendRequests || []).includes(user.username);
...
//   // Update both users: sender adds to sent, receiver adds to received
...
// app.post('/cancel-request', requireLogin, express.json(), async (req, res) => {
//   const { targetUsername } = req.body;
//   const currentUser = await usersCollection.findOne({ email: req.session.user.email });
...
//   const fromUser = await usersCollection.findOne({ email: req.session.user.email });
//   const toUser = await usersCollection.findOne({ username: toUsername });

//   if (!toUser || fromUser.username === toUsername) {
//     return res.status(400).send("Invalid request");
//   }

//   // Avoid duplicates
//   if (
//     toUser.friendRequestsReceived?.includes(fromUser.username) ||
//     toUser.friends?.includes(fromUser.username)
//   ) {
//     return res.status(409).send("Already sent or already friends");
//   }

//   await usersCollection.updateOne(
//     { username: toUsername },
//     { $addToSet: { friendRequestsReceived: fromUser.username } }
//   );
//   await usersCollection.updateOne(
//     { username: fromUser.username },
//     { $addToSet: { friendRequestsSent: toUsername } }
//   );
